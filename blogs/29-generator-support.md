...
1. **Variables to Struct fields**: `n` becomes `self->n` (survives across returns)
2. **Yield to Return + Label**: `yield n` becomes `return n` with a label after it
...
This section explains the C concepts needed to understand how generators work at the MicroPython runtime level.

### MicroPython's type system: `mp_obj_type_t`

Every Python object in MicroPython has a type, represented by `mp_obj_type_t`. This structure defines how the runtime interacts with objects:
...
struct _mp_obj_type_t {
    ...
    uint8_t slot_index_iter;        // Iterator behavior  <-- KEY FOR GENERATORS
...
The supported `range(...)` subset includes:

- `range(n)` where `n` is an int constant or a name
- `range(start, n)` where both are int constants or names
- `range(0, n, 1)` explicitly
...
```
1. Runtime calls our wrapper function `generators_countdown(3_obj)`
...
```
C: generators_countdown_gen_iternext(gen)
```
...
```
C: generators_countdown_gen_iternext(gen)  # Second call
```
