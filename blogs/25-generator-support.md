...
- `yield` is supported only as a statement, `yield` as an expression is not supported.
- `yield from` is not supported.
- `try`, `with`, and `async with` are not supported inside generator functions.
- `return <value>` is not supported inside generator functions.
- Generator methods (`send`, `throw`, `close`) are not implemented.
- `yield` inside non `range(...)` for loops is not supported.
- Supported generator `range(...)` subset is limited to: `range(n)`, `range(0, n)`, and `range(0, n, 1)` with no keywords.
