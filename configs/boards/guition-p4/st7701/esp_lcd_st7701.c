 ...
 * 
 * ST7701 MIPI-DSI driver for ESP32-P4
 * Adapted from esp-iot-solution esp_lcd_st7701 component
 */
...
// Default init sequence for 480x360 (will be overridden by vendor_config)
...
esp_err_t esp_lcd_new_panel_st7701(const esp_lcd_panel_io_handle_t io, const esp_lcd_panel_dev_config_t *panel_dev_config, esp_lcd_panel_handle_t *ret_panel)
{
    ...
    ESP_RETURN_ON_FALSE(vendor_config->flags.use_mipi_interface, ESP_ERR_NOT_SUPPORTED, TAG, "Only MIPI interface supported");
    ESP_RETURN_ON_FALSE(vendor_config->mipi_config.dpi_config && vendor_config->mipi_config.dsi_bus, ESP_ERR_INVALID_ARG, TAG, "invalid vendor config");
...
static esp_err_t panel_st7701_init(esp_lcd_panel_t *panel)
{
    ...
    if (id_ret == ESP_OK) {
        ...
        ESP_LOGW(TAG, "Failed to read LCD ID, continuing anyway");
    }
    ...
    for (int i = 0; i < init_cmds_size; i++) {
        if (is_command2_disable && (init_cmds[i].data_bytes > 0)) {
            ...
            if (is_cmd_overwritten) {
                is_cmd_overwritten = false;
                ESP_LOGW(TAG, "The %02Xh command has been used and will be overwritten", init_cmds[i].cmd);
            }
        }
...
static esp_err_t panel_st7701_mirror(esp_lcd_panel_t *panel, bool mirror_x, bool mirror_y)
{
    st7701_panel_t *st7701 = (st7701_panel_t *)panel->user_data;
    esp_lcd_panel_io_handle_t io = st7701->io;
    uint8_t sdir_val = mirror_x ? ST7701_CMD_SS_BIT : 0;
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, ST7701_CMD_SDIR, (uint8_t[]) { sdir_val }, 1), TAG, "send command failed");

    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, ST7701_CMD_CND2BKxSEL, (uint8_t []) {
        ST7701_CMD_BKxSEL_BYTE0, ST7701_CMD_BKxSEL_BYTE1, ST7701_CMD_BKxSEL_BYTE2, ST7701_CMD_BKxSEL_BYTE3, 0,
    }, 5), TAG, "send command failed");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, LCD_CMD_MADCTL, (uint8_t[]) { st7701->madctl_val }, 1), TAG, "send command failed");
